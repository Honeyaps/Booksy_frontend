...
class UserAPIService {
  ...
  static async getProducts(params) {
    const response = await axios.get(`${API_URL}/user/getProducts`, { params });
    return response.data;
  }
  ...
  static async addReview(data) {
    const headers = { Authorization: `Bearer ${localStorage.getItem('token')}` };
    const response = await axios.post(`${API_URL}/user/addReview`, data , { headers })
    return response.data;
  }

  static async getAllReviews(data) {
    const response = await axios.post(`${API_URL}/user/getAllReviews`, data)
    return response.data;
  }

  static async deleteReview(data) {
    const headers = { Authorization: `Bearer ${localStorage.getItem('token')}` };
    const response = await axios.post(`${API_URL}/user/deleteReview`, data , { headers })
    return response.data;
  }
